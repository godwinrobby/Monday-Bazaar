...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const [affiliateSaveStatus, setAffiliateSaveStatus] = useState<string | null>(null);
...
CREATE POLICY "Public deal_views access" ON public.deal_views FOR ALL USING (true) WITH CHECK (true);`;
  ...
  const handleUpdateAffiliateRule = async (store: StoreName, field: keyof StoreAffiliateConfig, value: any) => {
    setAffiliateConfigs(prev => {
      ...
      setAffiliateSaveStatus('Saving...');
      fetch('/api/affiliate-configs', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(updated)
      }).then(() => {
        setAffiliateSaveStatus('Saved');
        setTimeout(() => setAffiliateSaveStatus(null), 2000);
      }).catch(() => {
        setAffiliateSaveStatus('Save failed');
        setTimeout(() => setAffiliateSaveStatus(null), 3000);
      });
      return updated;
    });
  };
                 ...
                 <div className="flex items-center gap-2">
                   {affiliateSaveStatus && (
                     <span className={`px-2.5 py-1 rounded-lg text-[11px] font-bold ${
                       affiliateSaveStatus === 'Saved' ? 'bg-emerald-50 text-emerald-700' :
                       affiliateSaveStatus === 'Save failed' ? 'bg-red-50 text-red-700' :
                       'bg-amber-50 text-amber-700'
                     }`}>
                       {affiliateSaveStatus === 'Saved' && '✓ '}
                       {affiliateSaveStatus === 'Save failed' && '✕ '}
                       {affiliateSaveStatus}
                     </span>
                   )}
                   <button
                ...
                return (
                  <div key={store} className="bg-white p-5 rounded-3xl border border-slate-200 shadow-xs space-y-4">
                    <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                      ...
                      <div className="flex items-center gap-3">
                        <div className="flex items-center gap-1.5">
                          <span className="text-[10px] font-bold uppercase text-slate-400">Affiliate</span>
                        ...
                        <div className="flex items-center gap-1.5">
                          <span className="text-[10px] font-bold uppercase text-slate-400">Shop</span>
                          <label className="relative inline-flex items-center cursor-pointer">
                            <input
                              type="checkbox"
